...
Настроено монтирование volume ./data/ для обмена данными и результатами между сервисами.
...
Создан отдельный QuotesExportService, обеспечивающий сохранение котировок по каждому тикеру в формате JSON и CSV в общую папку data/quotes/ (volume, доступный для PHP и Python-контейнеров).
...
- Для взаимодействия между сервисами (PHP и Python) используется единая папка обмена данными — /data/:
    - PHP-скрипты сохраняют выгрузки данных (котировки, вакансии и пр.) в формате JSON в /data/quotes/, /data/vacancies/ и т.д.
    - Python-скрипты читают эти файлы, проводят анализ и сохраняют результаты обработки (например, отчёты или визуализации) в /data/results/.
...
### 23. Реализована страница альтернативных данных (AltData) с интеграцией HeadHunter API
- Создана страница "Альтернативные данные" с разметкой для вывода информации о вакансиях, новостях и событиях по каждой компании-эмитенту.
- Разработан PHP-клиент для интеграции с HeadHunter API (hh.kz) с поиском вакансий по отрасли (industry_id) в разрезе выбранных компаний (тикеров).
- Собираются и отображаются агрегированные метрики: топ работодателей, топ городов, распределение вакансий по времени публикации, зарплатные вилки.
- В случае ошибок обращения к API или отсутствия данных осуществляется логирование через Event (с детализацией причин и ответа сервера).
- Добавлен механизм выгрузки агрегированных данных по вакансиям в формате JSON (по аналогии с котировками, сохраняется в /data/vacancies/).
- Заложена архитектурная возможность расширения блока для новостей и событий (структура страницы и контроллера уже предусматривает эти секции).

Рекомендация (зафиксировано в истории):
- Кэширование результатов HeadHunter API не внедряется из-за ограничения по времени; при необходимости повторного анализа Python-скрипты будут использовать выгруженные JSON-файлы.

Краткая структура выгружаемого файла:
```
/data/vacancies/{SYMBOL}.json
[
  {
    "id": "...",
    "name": "...",
...
Пример выгрузки/сохранения:
```
// Пример функции
public static function saveVacanciesAsJson(array $vacancies, string $symbol, string $targetDir = '/data/vacancies/') {
...
```
### 24. Интеграция Airflow для автоматического экспорта вакансий и котировок в JSON
- Реализована автоматизация выгрузки альтернативных данных (вакансий) и котировок с помощью DAG Airflow и python-скриптов (get_all_vacancies.py, get_all_quotes.py).
- Данные по всем тикерам (компаниям) выгружаются ежедневно и сохраняются в /data/vacancies/ и /data/quotes/ в формате JSON.
- Весь процесс интегрирован через API PHP, что обеспечивает консистентность данных между фронтендом и аналитическими скриптами на Python.
- Проверена корректная работа DAG’ов (экспорт вакансий и котировок), подтверждено создание файлов, сняты скриншоты для диплома.
- Файлы формируются для всех компаний одновременно, что обеспечивает единый датасет для анализа в дипломной работе.
- Заложена архитектура для будущего кэширования: PHP/JS могут брать данные из этих файлов, минуя повторные запросы к внешним API.

Дополнительно:
- В TODO.md добавлен пункт о необходимости автоматического и ручного обновления выгруженных данных на сайте.
